...
exports.getDetailReservasi = (req, res) => {
  ...
  try {
    conn.query("SELECT reservasi.id_paket, reservasi.id_jadwal, reservasi.nama, reservasi.alamat, reservasi.total_pembayaran, reservasi.status_pembayaran, reservasi.bukti_pembayaran, paket.nama as nama_paket, paket.jarak, paket.waktu, paket.tarif, DATE_FORMAT(jadwal.jadwal, '%Y-%m-%d %H:%i:%s') AS jadwal FROM reservasi INNER JOIN paket ON reservasi.id_paket = paket.id INNER JOIN jadwal ON reservasi.id_jadwal = jadwal.id WHERE reservasi.id = ?", [id], (err, reservasi) => {
      if (err) return res.status(500).json({
        status: 'error',
        message: err
      });
