    ...
    <form class="row g-3">
      ...
      {fasilitas.map((val, key) => (
        <div class="input-group mb-3" style={{ padding: '0px 14px' }} key={key}>
          ...
          {val.value &&
            <button disabled={val.value === "" || key > 1} type='button' className="btn btn-primary input-group-text" id="basic-addon2" onClick={() => setFasilitas([...fasilitas, { id: fasilitas.length + 1, value: "" }])}>Tambah</button>
          }
        </div>
      ))}
